...
const RESOURCES = [
  ...
  { key: "oil", name: "Oil Pump", cost: 8000000, income: 2800000, breachDmg: 16000000, color: "#cc8800", icon: "O" },
...
const UNIT_SKETCHES = {
  ...
  resource: {
    ...
    oil: `<svg width="80" height="60" viewBox="0 0 80 60"><defs><linearGradient id="og" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#ee9900"/><stop offset="100%" stop-color="#885500"/></linearGradient></defs><line x1="30" y1="55" x2="30" y2="22" stroke="#fff" stroke-width="3"/><line x1="16" y1="55" x2="44" y2="55" stroke="#fff" stroke-width="3"/><line x1="30" y1="22" x2="60" y2="10" stroke="#fff" stroke-width="3"/><circle cx="30" cy="22" r="6" fill="url(#og)" stroke="#fff" stroke-width="2"/><rect x="56" y="6" width="12" height="16" rx="2" fill="url(#og)" stroke="#fff" stroke-width="1.5"/><line x1="62" y1="22" x2="62" y2="55" stroke="#fff" stroke-width="1.5" stroke-dasharray="3 3" opacity="0.4"/><circle cx="62" cy="55" r="3" fill="#cc8800" stroke="#fff" stroke-width="1"/></svg>`,
...
setAiSetup({ hqX: null, hqY: null, airspace: (THEATERS[theaterRef.current]?.airspace || [2000])[0], resources: [], interceptors: [], adUnits: [] });
  ...
  useEffect(() => {
    ...
    if (!battleActive) for (const dep of resourceDeposits) {
      ...
      const depIcons = {
        ...
        oil: `<svg width="${sz}" height="${sz}" viewBox="0 0 20 20"><line x1="10" y1="18" x2="10" y2="8" stroke="${sc}" stroke-width="${sw * 1.5}"/><line x1="6" y1="18" x2="14" y2="18" stroke="${sc}" stroke-width="${sw * 1.5}"/><line x1="10" y1="8" x2="16" y2="5" stroke="${sc}" stroke-width="${sw * 1.5}"/><circle cx="10" cy="8" r="2" fill="${res.color}" fill-opacity="${fop}" stroke="${sc}" stroke-width="${sw}"/><rect x="15" y="3" width="3" height="4" rx="0.5" fill="${res.color}" fill-opacity="${fop}" stroke="${sc}" stroke-width="${sw * 0.8}"/></svg>`,
    ...
    const resIcons = {
      ...
      oil: (c) => `<svg width="20" height="20" viewBox="0 0 20 20"><line x1="10" y1="18" x2="10" y2="8" stroke="#fff" stroke-width="1.5"/><line x1="6" y1="18" x2="14" y2="18" stroke="#fff" stroke-width="1.5"/><line x1="10" y1="8" x2="16" y2="5" stroke="#fff" stroke-width="1.5"/><circle cx="10" cy="8" r="2" fill="${c}" stroke="#fff" stroke-width="1"/><rect x="15" y="3" width="3" height="4" rx="0.5" fill="${c}" stroke="#fff" stroke-width="0.8"/></svg>`,
    ...
    if (aiSetup && aiSetup.hqX != null && aiSetup.hqY != null) {
      ...
      // Enemy airspace circle: always visible but faint when HQ is hidden (fog preview).
      // Full opacity when HQ is revealed. This gives players a general sense of enemy position.
      const airBreaches = battleRef.current?.aiAirBreaches || [];
      const airOp = hqVisible ? 0.9 : 0.15;
      const airFill = hqVisible ? 0.1 : 0.02;
      const airWeight = hqVisible ? 3 : 1;
      if (airBreaches.length === 0 || !hqVisible) {
        L.circle(toLL(aiSetup.hqX, aiSetup.hqY), { radius: aiSetup.airspace * mpu, color: opponentColor, fillColor: opponentColor, fillOpacity: airFill, weight: airWeight, opacity: airOp, dashArray: "10 6" }).addTo(layer);
      } else if (hqVisible) {
        const SEG = 24; const segArc = (Math.PI * 2) / SEG; const GAP = 0.15;
        for (let i = 0; i < SEG; i++) {
          const mid = -Math.PI + (i + 0.5) * segArc;
  ...
  const launchRound = useCallback(() => {
    // Use battleActiveRef for the freshest value (avoids stale closure from useCallback)
