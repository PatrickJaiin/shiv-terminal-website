...
// Sample run constants. The sample button fills the form with these values
// and Analyze short-circuits to the pre-computed JSON at /sample-analysis.json
// (generated by `npm run precompute-sample` from a residential IP) so the
// demo always works on Vercel even when YouTube blocks transcript fetching.
const SAMPLE_QUERY = "How do Google Maps actually works?";
const SAMPLE_VIDEO_ID = "c6wb9xtH7RM";
const SAMPLE_VIDEO_URL = "https://www.youtube.com/watch?v=c6wb9xtH7RM&t=232s";
...
export default function YouTubeAlgorithm() {
  ...
  // ─── cached sample replay ───────────────────────────────
  // Loads /sample-analysis.json (generated locally via
  // `npm run precompute-sample`) and walks the page through the same UI phase
  ...
  const runSampleFromCache = async (provider, presetId, depth) => {
    ...
    if (!res.ok) {
      throw new Error(
        `sample-analysis.json missing (status ${res.status}). Run \`npm run precompute-sample\` locally to generate it.`
      );
    }
    ...
    for (let i = 0; i < analysisResults.length; i++) {
      ...
      setProgress({
        msg: `Analyzing: ${(r.comparisonTitle || "").slice(0, 50)}...`,
        current: i + 1,
        total: analysisResults.length,
      });
      addLog(
        `[${i + 1}/${analysisResults.length}] Comparing with "${(r.comparisonTitle || "").slice(0, 60)}"`
      );
      addLog(`  Done -${Object.keys(r.text1 || {}).length} parameters scored`);
      await sleep(120);
    }
    setAnalysisResults(analysisResults);

    // Replay phase 3: stats + weight computation
    setPhase("scoring");
    ...
    // Phase 4: compute final scores from cached data using current parameters
    const weights = statsArr.map((s) => s.weight);
    const activePreset = PRESETS[presetId] || PRESET_YOUTUBE;
    const activeParamKeys = activePreset.params.filter((p) => p.enabled).map((p) => p.key);
    // Make sure UI parameters reflect the cache combo we're rendering
    setParameters(activePreset.params.map((p) => ({ ...p })));
    const scores = computeFinalScores(analysisResults, weights, activeParamKeys);
  ...
  const handleAnalyze = async () => {
    ...
    // Sample short-circuit: when the inputs match the canonical sample run
    // and the user is on one of the pre-computed presets, replay the cached
    // JSON instead of hitting the live APIs. This is the only path that
    // reliably works on Vercel since YouTube blocks transcript fetching from
    // cloud IPs.
    const isSampleInputs =
    ...
    try {
      ...
      if (results.length === 0) {
        ...
        if (userTranscriptFails.length > 0) {
          detail = `YouTube is blocking transcript access from this server's IP, which is a known limitation of running on Vercel/AWS. Click the Sample button above to view a pre-computed demo run, or clone the repo and run locally for a live analysis on your own video.`;
        } else if (transcriptFails === failures.length && transcriptFails > 0) {
          detail = `All ${transcriptFails} comparison videos had no fetchable transcripts. YouTube is blocking server-side transcript fetching from this host's IP (a known Vercel/AWS limitation). Click the Sample button above to view a pre-computed demo run, or clone the repo and run locally for a live analysis.`;
      ...
      <main className="pt-16">
        <article className="max-w-4xl mx-auto px-6 py-24">
          ...
          <div ref={demoRef} className="bg-gray-50 border border-gray-200 rounded-xl p-6 mb-12 scroll-mt-24">
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 text-[11px] text-amber-800 leading-snug mb-6">
              <span className="font-semibold">Heads up:</span> YouTube blocks transcript fetching from cloud server IPs, so live runs may fail on this hosted demo.
              {" "}<span className="font-semibold">Sample</span> is pre-cached and always works.
              For live analysis on your own video, clone the repo and run it locally.
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => {
                  setSearchQuery(SAMPLE_QUERY);
                  setVideoUrl(SAMPLE_VIDEO_URL);
                  // Cache only covers youtube/research presets. If the user
                  // is on custom, snap to youtube so the sample can replay.
                  if (preset === "custom") {
                    setPreset("youtube");
                    setParameters(PRESET_YOUTUBE.params.map((p) => ({ ...p })));
                  }
                }}
                disabled={isRunning}
                ...
                title="Fill with sample data, then click Analyze to view a pre-computed run"
              >
                Sample
              </button>
            </div>
