...
export async function ytFetch(urlBuilder, keys) {
  ...
  for (const key of keys) {
    ...
    if (isQuota) {
      deadKeys.add(key);
      lastError = new Error(`Quota exceeded for key ...${key.slice(-6)}`);
      continue;
    }
    throw new Error(
      `YouTube API error: ${res.status} ${errBody?.error?.message || ""}`.trim()
    );
  }
  throw lastError || new Error("No YouTube API keys available");
}
