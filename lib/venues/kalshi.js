...
// Series tickers per sport. Tried in order, deduped by market ticker. When
// none return markets, we fall back to /events keyword search with the
// strings below.
const SERIES_TICKERS = {
  nba: ["KXNBAGAME", "KXNBA"],
  ipl: ["KXIPL", "IPL", "KXCRICKETIPL"],
  lol: ["KXLOLMATCH", "KXESPORTSLOL", "KXLOL", "KXLCK", "KXLEC", "KXLCS", "KXLPL", "KXWORLDS"],
  val: ["KXVALMATCH", "KXESPORTSVAL", "KXVAL", "KXVCT"],
};
...
export function buildKalshiAdapter({ auth, apiBase = DEFAULT_BASE, fetchImpl } = {}) {
  ...
  async function listMarkets({ sport }) {
    ...
    const keywords = EVENT_SEARCH_KEYWORDS[sport] || [];
    /** @type {Map<string, any[]>} eventTicker -> rawMarkets */
    const byEvent = new Map();
    const diag = { triedSeries: tickers.length, triedKeywords: 0, seriesHits: 0, keywordHits: 0 };

    // Strategy 1: series-ticker filter on /markets.
    for (const series of tickers) {
      try {
        const resp = await doFetch(`${apiBase}/markets?series_ticker=${series}&status=open&limit=200`, {});
        const data = await resp.json();
        const markets = data.markets || [];
        diag.seriesHits += markets.length;
    ...
    // Strategy 2: /events keyword search. Runs unconditionally because
    // Kalshi splits esports into many series we can't enumerate up front,
    // and because IPL/NBA sometimes surface under new series names.
    for (const keyword of keywords) {
      diag.triedKeywords++;
      try {
        const resp = await doFetch(`${apiBase}/events?status=open&limit=200`, {});
        const data = await resp.json();
        const events = (data.events || []).filter((e) => {
          const s = ((e.title || "") + " " + (e.sub_title || "") + " " + (e.category || "")).toLowerCase();
          return s.includes(keyword);
        });
        for (const ev of events) {
          for (const m of (ev.markets || [])) {
            const et = eventTickerFromMarketTicker(m.ticker);
            if (!byEvent.has(et)) byEvent.set(et, []);
            if (!byEvent.get(et).some((x) => x.ticker === m.ticker)) {
              byEvent.get(et).push(m);
              diag.keywordHits++;
            }
          }
        }
