...
export async function findOpportunities({ adapters, sport, cfg: userCfg = {} }) {
  ...
  // 1. Pull normalized markets from both venues in parallel.
  const [kMarkets, pMarkets] = await Promise.all([
    kalshi.listMarkets({ sport }).catch((e) => { return { __err: e.message }; }),
    polymarket.listMarkets({ sport }).catch((e) => { return { __err: e.message }; }),
  ]);
  const kList = Array.isArray(kMarkets) ? kMarkets : [];
  const pList = Array.isArray(pMarkets) ? pMarkets : [];
  ...
  await Promise.all(pairs.map(async ({ a: kMkt, b: pMkt }) => {
    // Determine which Kalshi ticker corresponds to which canon team.
    const kAIsTeamA = canonOf(kMkt.teamA.canon) === canonOf(pMkt.teamA.canon);
    ...
    //
    //   Direction 1: BUY team on Kalshi (YES), BUY opponent on Polymarket.
    //     Kalshi YES asks vs Polymarket opponent asks.
    //     gross = 1 - kalshiYesAsk(team) - polyAsk(opponent)
    //
    //   Direction 2: BUY team on Polymarket, BUY opponent on Kalshi YES.
    //     Polymarket ask on team + Kalshi YES ask on opponent.
    //     gross = 1 - polyAsk(team) - kalshiYesAsk(opponent)
    //
    // These are the only two legal binary hedge pairs. We evaluate both and
    // emit the better one per slot.

    // Pre-decide target contracts from a speculative 1-unit vwap on best-case
    // direction, then we lock in a conservative size using min-depth.
    ...
    const dir1 = evalDirection({
      ...
      eventLabel: kMkt.label,
      teamAMeta: { venue: "kalshi", marketId: kTickerForA, side: "YES", backsTeamCanon: kMkt.teamA.canon },
      teamBMeta: { venue: "polymarket", marketId: pMkt.marketId, tokenId: pTokenB, side: "YES", backsTeamCanon: kMkt.teamB.canon },
    ...
    const dir2 = evalDirection({
      ...
      eventLabel: kMkt.label,
      teamAMeta: { venue: "polymarket", marketId: pMkt.marketId, tokenId: pTokenA, side: "YES", backsTeamCanon: kMkt.teamA.canon },
      teamBMeta: { venue: "kalshi", marketId: kTickerForB, side: "YES", backsTeamCanon: kMkt.teamB.canon },
      kalshi, polymarket, cfg,
    });
